...
def main():
    ...
    while True:  # Validates the number
        ...
        elif not pizzas.isnumeric():
            pizzas = input(
                "Please input a number, not a letter or symbol. Please in put a number between 1 and 10: ")
        else:
            break
    i = 0
    ...
    while i <= int(pizzas) - 1:  # Loops for number of pizzas and gets information of each pizza
        ...
        unique_code = input("Please input a three letter/number menu code for pizza number " + str(
            i + 1) + ": ")  # Gets unique code from user
        ...
        stock = input("Please input the amount of the " + str(
            name) + " pizza you want in the virtual storage freezer: ")  # Gets amount of the type of pizza
        while True:  # Validates the number
        ...
        i += 1
    to_string(order, pizzas)  # Prints using the to_string function
